...
resource "aws_instance" "bastion" {
  ...
  subnet_id     = "${aws_subnet.ocp_pub_subnet.0.id}"
  ...
  connection {
    host          = "${self.public_ip}"
    user          = "ec2-user"
    private_key   =  "${file("~/Downloads/vbudi-key-east-2.pem")}"
  }
  ...
  inline = [
    ...
    "git clone https://github.com/ibm-cloud-architecture/terraform-openshift4-aws"
  ]
  }
}
